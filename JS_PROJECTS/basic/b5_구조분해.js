...
// 전달된 데이터가 json or 객체 => 객체 구조 분해 => 원하는값 추출
let data = {
    s1:'서면역',
    s2:'부전역',
    s3:'부산역',
    s4:['사상역', '김해공항역', '센텀시티역'] // 배열은 Array객체
}
console.log( data )
// 값 추출
console.log( data.s2 )
...
s3 = '변경' // 원본에 영향을 미지치 않는다 => 사본이다
console.log( s3, data)

// ===================================================================
// 배열(순서가 존재, 인덱스사용(0,1,2,..)) 구조 분해
// ['사상역', '김해공항역', '센텀시티역']
console.log( s4, s4[0] )
// 쓰고 싶은 변수명을 사용하여 추출 -> 순서가 중요
// 추출않하는 자리는 ,로 대체, 활용 빈도가 높지는 않다
let [ , , city ] = s4
console.log( city )
...
// 전역변수의 값(변수들)들을 객체 생성시 데이터로 사용가능하다
let obj = {
    w:w1,
    age:age1
}
...
// 여러 객체를 카피해서 새로운 객체를 만들때, 중복되는 값이 있다면 뒤에것으로 대체된다
